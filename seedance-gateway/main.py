...
def get_admin_access_token() -> str:
    return os.getenv("ADMIN_ACCESS_TOKEN") or get_gateway_access_token()
...
def verify_admin_token(authorization: str | None = Header(None)) -> None:
    ...
    expected = f"Bearer {admin_access_token}"
    if authorization is None or not hmac.compare_digest(authorization, expected):
...
async def add_admin_security_headers(request: Request, call_next):
    ...
    if request.url.path == "/admin" or request.url.path.startswith("/admin/api/"):
...
async def admin_page():
    with (TEMPLATE_DIR / "admin.html").open("r", encoding="utf-8") as f:
        return f.read()
...
async def get_provider_detail_api(provider_slug: str):
    store = await get_provider_store()
    try:
        provider = await store.get_provider(provider_slug)
    except ProviderNotFoundError as exc:
